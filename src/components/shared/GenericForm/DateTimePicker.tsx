...
const DatePickerComponent: React.FC<DatePickerProps> = ({
  ...
  // Sync tempDate with controllerField.value (e.g. when form resets)
  useEffect(() => {
  ...
  const toggleDropdown = () => {
    if (!readOnly) {
      if (!isOpen && containerRef.current) {
        ...
        const dropdownHeight = 450; // Estimated max height of dropdown
  ...
  const getDisplayValue = () => {
    if (controllerField.value) {
      try {
        ...
        console.error("Invalid date value:", controllerField.value);
      }
    }
  ...
  return (
    ...
    >
      ...
      <input type="hidden" {...controllerField} value={controllerField.value || ""} />
      ...
      {isOpen && !readOnly && (
        <div
          ref={dropdownRef}
          className={`absolute z-[100] ${openUpward ? "bottom-full mb-2" : "mt-2"} bg-white dark:bg-slate-800 border border-slate-300 dark:border-slate-600 rounded-xl shadow-2xl shadow-black/40 overflow-visible min-w-[320px]`}
          style={{
            [isRTL ? "right" : "left"]: 0,
          ...
          <div className="p-4 border-t border-slate-200 dark:border-slate-700 bg-gradient-to-r from-slate-50 to-blue-50 dark:from-slate-800 dark:to-blue-900/20">
            <div className="flex justify-end items-center">
              <button
                ...
                className="px-6 py-2.5 text-sm font-bold bg-gradient-to-r from-blue-600 to-indigo-600 hover:from-blue-700 hover:to-indigo-700 text-white rounded-xl shadow-lg shadow-blue-500/20 transition-all duration-300 hover:scale-[1.02] active:scale-[0.98]"
              >
                {t("common.apply")}
              </button>
            </div>
